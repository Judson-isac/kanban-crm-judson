class Api::V1::Accounts::KanbanItemsController < Api::V1::Accounts::BaseController
  before_action :fetch_kanban_item, except: [:index, :create, :reorder, :debug, :reports, :search, :filter, :export, :import, :import_preview,
                                           :bulk_move_items, :bulk_assign_agent, :bulk_set_priority]
  ...
  def index
    ...
    @conversation_display_id ||= permitted_params[:conversation].presence
  ...
  def filter
    ...
    # Filtrar por conversa (conversation_display_id)
    if permitted_params[:conversation].present?
      base_query = base_query.where(conversation_display_id: permitted_params[:conversation].to_s)
    end
  ...
  def show
    ...
    data = Rails.cache.fetch(cache_key, expires_in: 5.minutes) do
      ...
      # Garantir que as notas, offers e custom_attributes estejam completas (array para o frontend)
      if json_data['item_details'].present?
        ...
        raw_ca = @kanban_item.item_details['custom_attributes']
        json_data['item_details']['custom_attributes'] = raw_ca.is_a?(Array) ? raw_ca : []
      end
  ...
  def create
    ...
    apply_default_offer_value(@kanban_item)
  ...
  def destroy
    authorize @kanban_item
    webhook_service.notify_item_deleted(@kanban_item)
    @kanban_item.destroy!
    head :ok
  end
  ...
  def fetch_items_with_cache
    @items.map do |item|
      ...
      Rails.cache.fetch(cache_key, expires_in: 5.minutes) do
        ...
        # Garantir que as notas, offers, custom_attributes e checklist estejam completas (array para o frontend)
        if json_data['item_details'].present?
          json_data['item_details']['notes'] = item.item_details['notes'] || []
          json_data['item_details']['offers'] = item.item_details['offers'] || []
          raw_ca = item.item_details['custom_attributes']
          json_data['item_details']['custom_attributes'] = raw_ca.is_a?(Array) ? raw_ca : []
        end
  ...
  def apply_default_offer_value(kanban_item)
    details = kanban_item.item_details || {}
    offers = details['offers']
    return unless offers.is_a?(Array) && offers.any?

    default_offer = offers.find { |o| o.is_a?(Hash) && o['default'] == true }
    default_offer ||= offers.first
    return unless default_offer.is_a?(Hash)

    details['value'] = default_offer['value']
    details['currency'] = default_offer['currency']
  end
  ...
  def build_base_query
    ...
    query = query.where(conversation_display_id: @conversation_display_id) if @conversation_display_id.present?
    query
  end
